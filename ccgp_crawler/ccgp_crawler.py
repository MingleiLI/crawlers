...
class crawl_ccgp(object):
    ...
    def crawl_list(self, target_link, outfile):
        ...
        for item in items:
            ...
            text = item.find_element_by_xpath('//a').text.strip()
            loc = item.find_element_by_xpath('//span/a').text.strip()
            print loc
            print >> outfile, str(text.encode('utf-8')) + '|' + str(link) + '|' + str(loc)
...
if __name__ == "__main__":
    ...
    outfile = open('ccgp_list_loc_' + str(cur_thr_index) + '.txt', 'a')
    i = start_index
