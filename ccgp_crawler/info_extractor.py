...
#zbgg = ['招标编号', '采购编号','项目名称', '委托单位', '集中采购机构全称', '集中采购机构地址', '采购人地址', '项目负责人' ,'招标范围及形式', '简要技术要求 /招标项目的性质', '评标方法和标准',    '价格', '预算资金',  '采购方式', '采购用途', '询标时间', '询标内容', '询标地点', '投标截止时间、开标时间', '投标、开标地点',   ]

zbgg = ['招标公告编号', '招标编号', '采购编号','项目名称', '集中采购机构全称', '集中采购机构地址','采购人名称', '采购人地址',  '采购用途', '采购方式', '招标范围及形式' , '招标公告日期' , '定标日期','合同履行日期', '预算资金', '中标供应商名称', '中标供应商地址', '中标金额', '评标委员会成员名单','项目负责人']


gg_content = '招标货物名称及数量'
...
for file_name in files:
    ...
    for n, line in enumerate(info):
        ...
        line[0] = line[0].strip()
        line[1] = line[1].strip()
        if line[0] == gg_content:
            cont_tmp = line[1].strip()
            next_line = info.next()
            while ':' not in next_line and '：' not in next_line:
                cont_tmp = cont_tmp + ' ' + next_line.strip()
                next_line = info.next()
            cont_tmp = cont_tmp.strip()
            if cont_tmp != '':
                gg_info[-1] = cont_tmp.strip()
